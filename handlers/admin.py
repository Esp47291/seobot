...
@router.callback_query(F.data == "admin:reviews_passed")
async def reviews_passed(cb: CallbackQuery, **data):
    await cb.answer()
    session = data["session"]
    task_repo = TaskItemRepository(session)
    user_repo = UserRepository(session)
    ...
    rows = await session.execute(
        select(Attempt).where(Attempt.status == "completed").order_by(Attempt.id.desc()).limit(30)
    )
    attempts = list(rows.scalars().all())
    if not attempts:
        await cb.message.answer("Пока нет прошедших (принятых) отзывов.", reply_markup=admin_back_main_kb())
        return

    await cb.message.answer(f"🕘 Прошедшие отзывы (последние {len(attempts)}):")
    for at in attempts:
        u = await user_repo.get_by_user_id(at.user_id)
        uname = f"@{u.username}" if u and u.username else "—"
        task = await task_repo.get_by_id(at.task_item_id)
        ...
        completed_line = "—"
        if getattr(at, "updated_at", None):
            completed_line = at.updated_at.strftime("%d.%m.%Y %H:%M") + " UTC"
    ...
    await cb.message.answer("◀ Назад в админ-меню", reply_markup=admin_back_main_kb())
