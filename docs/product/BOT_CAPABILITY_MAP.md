...
- `origin/patch-15` HEAD всё ещё `a768bf3`. Зоны выплат / рефералка / FSM / channel gate / CSV / сайт без изменения кода.
- Copy-of-record (не секрет): `RULES_TEXT` = «платит ВАМ за отзывы» + пассив + кнопка прокси с захардкоженным URL; welcome default «начнем зарабатывать»; реферал КАПС; WD paid без суммы; `review_ok` admin-path «принят и оплачен» при credited; mgr-path честнее до `mgr_outpay`.
- Prebuilt: при `admin:allow` исполнителю уходит «Готовый текст для отзыва» — это не витрина сайта, это stuffing в продукте. COPY-01: не слать, не переименовывать.
- Сайта нет. Код бота не меняли. Инвентарь: `docs/product/COPY_SYSTEM.md`.
...
## Инкремент аудита 2026-08-24 H1 круг 2 (~07:51 UTC)

- `origin/patch-15` HEAD всё ещё `a768bf3`. Зоны выплат / рефералка / FSM / channel gate / CSV / сайт без изменения кода.
- После скрина (user.py): профиль «Ожидайте решение»; отзыв «Ожидайте проверки»; 2-й акк «Ожидайте решения» — везде без attempt id. Кабинет `_review_status_line`: completed/rejected/review_submitted; допуск = сырой status.
- Хаб admission по-прежнему без `login_screenshot` → нельзя обещать позицию в очереди. `MemoryStorage` на ожидании = деплой стирает FSM, не статус Attempt в БД.
- Сайта нет. Код бота не меняли. Spec: `BOT_UX_VISION.md` UX-03.
